loadAnnotations <- function(lib) {
   
   lib.db <- paste(lib, "db", sep='.')
   require (lib.db, character.only=T)  
   
   lib.sym <- paste(lib, "SYMBOL", sep='')
   p<-as.list(get(lib.sym))
   lib.sym <- paste(lib, "sym", sep='.')
   assign(lib.sym,,data.frame(affy.id=names(p),symb=as.character(p)),pos=1)
