getKEGGchisq <- function (gpl, deg, alf) {
...
#a = number of "successes" in sample (O)
#b = size of sample (dk)
#c = number of "successes" in population (E)
#d = size of population (ak)
#binom.test (a, b, p=c/d)
#prop.test  (c(a,c), c(b,d))
   ...
   for ( i in 1:nrow(deg.tab) ) {
     ...
     p <- prop.test(c(O,E),c(dk,ak))   
     deg.chi <- c(deg.chi, p$p.value)
   }
