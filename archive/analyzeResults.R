...
analyzeResults <- function (fit3, aff.chip, pval=.05, mval=1, adjm="none") {
...
cat("analyzeResults() step 1 completed, fit4 created.\n")
...
cat("analyzeResults() step 2 completed, fit4.top created.\n")

# Step 3:  Add annotations to matrix.
   if (!is.na(aff.chip)) { fat <- addEGID(fit4,aff.chip)
   } else                { fat<-fit4 } 
cat("analyzeResults() step 3 completed, fat created.\n")
   return(fat)
}